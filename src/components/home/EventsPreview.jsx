        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {previewEvents.map((event, index) => (
            <ScrollReveal key={event.id} delay={index * 150} className="h-full">
              <Link to={`/events/${event.id}/tickets`} className="event-card rounded-2xl overflow-hidden group block h-full bg-[#111] hover:bg-[#1a1a1a] transition-colors flex flex-col">
                <div className="relative aspect-[4/3] sm:aspect-[4/5] bg-black border-b border-white/5">
                  <img
                    src={event.image || event.cardImage}
                    alt={event.title}
